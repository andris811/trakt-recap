...
export default function SeasonModal({ showId, showTitle, season, episodes, watchedEpisodes, onClose, onOpenEpisode }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-800 w-full max-w-lg max-h-[80vh] flex flex-col mx-2 sm:mx-0">
        <div className="p-4 sm:p-6 border-b border-zinc-800">
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <h2 className="text-lg sm:text-xl font-bold text-white truncate">{showTitle}</h2>
              <p className="text-zinc-400 text-xs sm:text-sm mt-1">Season {season.number} &middot; {episodes.length} episodes</p>
            </div>
